...
#define DEVICE_NAME	"mod_desenc"//nombre--->aparece en /proc/devices
...
static int my_open(struct inode *i, struct file *f)
{
	...
	printk(KERN_INFO "desencriptador_code: open()\n");
	if(down_interruptible(&virtual_device.sem)!=0){
	printk(KERN_ALERT "desencriptador_code: no se pudo acceser al dispositivo durante la apertura.\n");
	return -1;
	}
	printk(KERN_ALERT "desencriptador_code: Dispositivo abierto.\n");
	return 0;
}
static int my_close(struct inode *i, struct file *f)
{
	up(&virtual_device.sem);
	printk(KERN_INFO "desencriptador_code: archivo cerrado.\n");
	return 0;
}
...
{
	ssize_t bytes;
	printk(KERN_INFO "desencriptador_code: read()\n");
...
{
	size_t index;
	printk(KERN_INFO "desencriptador_code: write()\n");
 	...
 	*off += len;
	printk(KERN_INFO "desencriptador_code: se escribio:%s.\n",virtual_device.data);
	
	index=0;
	while(index<len){
		virtual_device.data[index]=virtual_device.data[index]-1;//Copia el la información desencriptada
		index++;
	}
...
static int __init ofcd_init(void) /* Constructor */
{
	...
	printk(KERN_INFO "desencriptador_code: ofcd registrado.");
	if ((ret = alloc_chrdev_region(&first, 0, 1, DEVICE_NAME)) < 0)
	{
		return ret;
	}
	if (IS_ERR(cl = class_create(THIS_MODULE, "desencriptador")))
	{
...
static void __exit ofcd_exit(void) /* Destructor */
{
	...
	class_destroy(cl);
	unregister_chrdev_region(first, 1);
	printk(KERN_INFO "desencriptador_code: modulo descargado.");
}
